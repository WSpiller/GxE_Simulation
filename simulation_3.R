...
for(m in c(1,5,10,25,50,75,100)){
  ...
  G_beta<-c(G_beta,median(GENIUS_beta))
  G_se<-c(G_se,median(GENIUS_se))
  medFvec<-c(medFvec,median(res[,3]))
  F14<-c(F14,res[14,3])
  GxE_beta<-c(GxE_beta,res[14,1])
  GxE_se<-c(GxE_se,res[14,2])
  bpb_out<-c(bpb_out,median(bp_pval))
  
}

all_out<-data.frame(G_beta,G_se,GxE_beta,GxE_se,medFvec,F14,bpb_out)
